...
- [phase-3-subitems-ui] Subitem assignee endpoints (`server/routes/tasks.js:6497/6513/6534`) are missing `assertWorkspaceAccess` — any authenticated staff user can list/add/remove assignees on any subitem if they have the id. Drawer surfacing now exposes this, so harden in an auth-audit pass.
- [phase-3-subitems-ui] Subitem dependency endpoints (`server/routes/tasks.js:6549/6573/6607/6619`) likewise have no workspace access check and DELETE doesn't verify the dep belongs to a subitem in a workspace the caller can access. Same auth-audit pass.
- [phase-3-subitems-ui] Subitem cycle-detection on `POST /subitems/:subitemId/dependencies` runs the DFS outside any transaction and INSERTs without `FOR UPDATE` — same race as the item-level dependency aspect already in known_issues.
- [phase-3-subitems-ui] Archived subitems are not exposed in the new drawer tab. `?includeArchived=true` and `POST /subitems/:subitemId/restore` already exist on the backend and the hook ships `handleRestoreSubitem`; add an "Archived" toggle on the Subitems tab so admins can recover archived rows without a DB shell.
- [phase-3-subitems-ui] No way yet to schedule subitems: `start_date` and `due_date` are accepted by `subitemUpdateSchema` but the drawer row only renders status/assignees. Either add a compact date pair inline or a per-row expansion. Skipped here to keep the row a single line.
- [phase-3-subitems-ui] Subitem dependencies are not visualized in the drawer either. The endpoints exist (predecessors / successors / blocked-status) but rendering them needs more vertical space than a row can give; consider a per-row expander in a follow-up.
- [phase-4-bulk-select] Bulk paths deliberately skip per-item `createNotification` / email fan-out on assignee-add so a 50-item bulk-assign doesn't fire 50 push notifications. Replace with a single summary notification per bulk action ("You were assigned to 12 tasks in *Client X* board" → deep link). Needs a new notification shape and email template — out of scope for the aspect.
- [phase-4-bulk-select] No undo affordance for bulk archive. The single-row archive UX has a `POST /items/:itemId/restore` counterpart but no undo button; a snackbar-style "Archived 12 items — Undo" backed by a new `POST /items/bulk-restore` would round out the flow.
- [phase-4-bulk-select] Bulk assignee across mixed workspaces returns a hard 403 with no per-item feedback when the target user isn't a member of every touched workspace. `BoardTable` is single-board (therefore single-workspace) so this shouldn't fire in practice; the endpoint check exists to prevent a tampered `item_ids` payload. If the UI ever surfaces multi-board selection, teach the endpoint to report which items were skipped instead of failing the whole request.
- [phase-4-bulk-select] `POST /items/bulk-labels` on an `is_exclusive` label runs `DELETE` (clear old category) + `INSERT ... ON CONFLICT DO NOTHING` inside a single transaction, but under `READ COMMITTED` a concurrent single-item `POST /items/:itemId/labels` could re-insert the just-deleted row between the two statements. Same class of race as the item-dependency cycle-detection already in `known_issues`; fix belongs in a dedicated concurrency pass.

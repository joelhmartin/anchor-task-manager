...
- [phase-3-files-delete-preview] Remove `app.use('/uploads', express.static(UPLOAD_DIR))` from `server/index.js` once no `task_files.file_url LIKE '/uploads/%'` rows remain. Currently those rows still serve bytes unauthenticated whenever a container instance happens to still have the disk file. Pair the removal with a backfill/cleanup migration that nulls the `file_url` (or hard-deletes the row) for orphans.

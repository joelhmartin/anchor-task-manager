...
export default function NavItem({ item, level, isParents = false, setSelectedID }) {
  ...
  if (typeof item?.isActive === 'function') {
    isSelected = item.isActive({ pathname, search });
  } else {
    isSelected = !!matchPath({ path: item?.link ? item.link : item.url, end: false }, pathname);
  }
  ...
  const collapsedIcon = !drawerOpen && level === 1;
    ...
    <>
      ...
      >
        <ButtonBase
          ...
          sx={{ borderRadius: collapsedIcon ? '50%' : `${borderRadius}px` }}
          disableRipple={drawerOpen}
        >
          <ListItemIcon
            sx={{
